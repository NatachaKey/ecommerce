...
const createOrder = async (req, res) => {
  ...
  const { items: cartItems, tax, shippingFee, subtotal } = req.body;
  ...
  const { subtotal: subtotalCalculated, orderItems } = await cartItems.reduce(
    processOrder,
    initialValue
  );

  //calculate order total
  const total = tax + shippingFee + subtotalCalculated;
